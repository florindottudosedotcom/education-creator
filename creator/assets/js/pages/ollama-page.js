/**
 * Ollama Page Controller
 * Coordinates UI and business logic for local server AI
 */
...
class OllamaPage {
  ...
  bindAuthEvents() {
    ...
    connectBtn?.addEventListener('click', async () => {
      ...
      if (success) {
        ...
        this.showError('Failed to connect. Make sure Ollama is running.');
      }
    });
  ...
  updateModelList() {
    const modelSelect = document.getElementById('model-select');
    if (!modelSelect || this.provider.availableModels.length === 0) return;

    // Clear existing options
    modelSelect.innerHTML = '';

    // Add available models
    this.provider.availableModels.forEach(model => {
  ...
  restoreUIState() {
    ...
    const serverUrlInput = document.getElementById('server-url');
    if (serverUrlInput) {
      serverUrlInput.value = this.provider.baseURL;
    }
