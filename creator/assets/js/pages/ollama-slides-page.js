/**
 * Ollama Slides Page Controller
 * Coordinates UI and business logic for local Ollama presentation creation
 */
...
class OllamaSlidesPage {
  ...
  bindAuthEvents() {
    ...
    connectBtn?.addEventListener('click', async () => {
      ...
      this.showStatus('connection-status', 'Connecting to Ollama...', 'info');

      const success = await this.provider.connect({ url: serverUrl });
      this.setLoading(connectBtn, false);

      if (success) {
        this.showSuccess('Connected to Ollama successfully!');
        this.updateConnectionUI(true);
      } else {
        this.showError('Failed to connect to Ollama server.');
      }
    });
    ...
    disconnectBtn?.addEventListener('click', () => {
      ...
      this.showSuccess('Disconnected from Ollama');
    });
  ...
  async enhanceTopic() {
    ...
    if (!this.provider.isConnected()) {
      this.showError('Please connect to Ollama first');
      return;
    }
  ...
  restoreUIState() {
    ...
    // Restore server URL
    const serverUrlInput = document.getElementById('server-url');
    if (serverUrlInput && this.provider.config?.url) {
      serverUrlInput.value = this.provider.config.url;
    }
